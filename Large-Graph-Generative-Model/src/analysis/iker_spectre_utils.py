...
from src.analysis.dist_helper import compute_mmd, gaussian_emd, gaussian, emd, gaussian_tv, disc, gaussianRBF
from torch_geometric.utils import to_networkx
import wandb
from src.analysis.bilaketaHeuristikoak import simulatedAnnealing_deia, EDA_deia, funtzioEraikitzaile_deia
...
# Compute the solutions of instances in reference and predicted sets of graphs.
# The solutions will be calculated using differnt kernels (EDA, SA and constructive function)
def emaitzakLortuEbatzaileekin(sample_ref = None, sample_pred = None, kernel = EDA_deia, is_parallel = False):
    """Compute the solution of the problems using different kernels"""
    ...
    graph_ref_list_remove_empty = [G for G in sample_ref if not G.number_of_nodes() == 0]
    graph_pred_list_remove_empty = [G for G in sample_pred if not G.number_of_nodes() == 0]
    

    
    if is_parallel:
        with concurrent.futures.ProcessPoolExecutor() as executor:
            refList1 = list(executor.map(kernel, graph_ref_list_remove_empty))
        with concurrent.futures.ProcessPoolExecutor() as executor:
            predList1 = list(executor.map(kernel, graph_pred_list_remove_empty))
    else:
        for i in range(len(graph_ref_list_remove_empty)):
            refList1.append(kernel(graph_ref_list_remove_empty[i]))
        for i in range(len(graph_pred_list_remove_empty)):
            predList1.append(kernel(graph_pred_list_remove_empty[i]))
        

    return refList1, predList1

# A function to calculate Kullback–Leibler divergence between two probability distributions
def kl_divergence(a, b):
    a_luze = len(a)
    b_luze = len(b)
    # KL divergence-an hobe da 0-rik ez egotea
    all_keys = set(a.keys()).union(set(b.keys()))

    # Create new dictionaries A' and B' with missing keys filled with 0
    a_prime = {key: a.get(key, 0) for key in all_keys}
    b_prime = {key: b.get(key, 0) for key in all_keys}

    a_probs = [val for _, val in a_prime.items()]
    b_probs = [val for _, val in b_prime.items()]

    return sum(a_probs[i] * np.log(max(a_probs[i] / (b_probs[i] + 1e-10), 1e-10)) for i in range(len(a_probs)) if a_probs[i] > 0)


# calculate MMD between reference and predicted lists solution. The method to be used is EDA
def EDA_MMD_stats(graph_ref_list, graph_pred_list, is_parallel=True):
    ref, pred= emaitzakLortuEbatzaileekin(sample_ref = graph_ref_list, sample_pred = graph_pred_list, kernel=EDA_deia, is_parallel = is_parallel)
    mmd_dist = compute_mmd(ref, pred, kernel = gaussianRBF, is_hist=False)

    return mmd_dist

# calculate MMD between reference and predicted lists solution. The method to be used is Simulated Annealing
def SA_MMD_stats(graph_ref_list, graph_pred_list, is_parallel=True):
    ref, pred= emaitzakLortuEbatzaileekin(sample_ref = graph_ref_list, sample_pred = graph_pred_list, kernel=simulatedAnnealing_deia, is_parallel = is_parallel)
    mmd_dist = compute_mmd(ref, pred, kernel = gaussianRBF, is_hist=False)

    return mmd_dist

# calculate MMD between reference and predicted lists solution. The method to be used is constructive function
def FE_MMD_stats(graph_ref_list, graph_pred_list, is_parallel=True):
    ref, pred= emaitzakLortuEbatzaileekin(sample_ref = graph_ref_list, sample_pred = graph_pred_list, kernel=funtzioEraikitzaile_deia, is_parallel = is_parallel)
    mmd_dist = compute_mmd(ref, pred, kernel = gaussianRBF, is_hist=False)

    return mmd_dist


# calculate KL between reference and predicted lists solution. 
def KL_stats(graph_ref_list, graph_pred_list, is_parallel=True, kernel = EDA_deia):
    ref, pred= emaitzakLortuEbatzaileekin(sample_ref = graph_ref_list, sample_pred = graph_pred_list, kernel=kernel, is_parallel = is_parallel)
    freq_ref = Counter(ref)
    freq_pred = Counter(pred)

    # Normalize counts to get probabilities
    total_ref = sum(freq_ref.values()) 
    probs_ref = {k: v / total_ref for k, v in freq_ref.items()} 

    total_pred = sum(freq_pred.values()) 
    probs_pred = {k: v / total_pred for k, v in freq_pred.items()} 
    
    return kl_divergence(probs_ref, probs_pred)

# Calculate KL between reference and predicted solutions using EDA
def EDA_KL_stats(graph_ref_list, graph_pred_list, is_parallel=True):
    return KL_stats(graph_ref_list, graph_pred_list, is_parallel=is_parallel, kernel = EDA_deia)

# Calculate KL between reference and predicted solutions using Simulated Annealing
def SA_KL_stats(graph_ref_list, graph_pred_list, is_parallel=True):
    return KL_stats(graph_ref_list, graph_pred_list, is_parallel=is_parallel, kernel = simulatedAnnealing_deia)

# Calculate KL between reference and predicted solutions using Constructive function
def FE_KL_stats(graph_ref_list, graph_pred_list, is_parallel=True):
    return KL_stats(graph_ref_list, graph_pred_list, is_parallel=is_parallel, kernel = funtzioEraikitzaile_deia)
...
def calculate_overall_ged_metrics(predicted_graphs, reference_graphs, ged_cost_settings=None, is_parallel=True):
    ...
    return {
        "min_ged": final_min_ged,
        "max_ged": final_max_ged,
        "mean_ged": final_mean_ged,
        "std_ged": final_std_ged,
        "median_ged": final_median_ged # Optionally return the full list
    }

# --- Example Usage ---
def GED_stats(graph_ref_list, graph_pred_list, is_parallel=True, compute_emd=False):
...
def clustering_stats(graph_ref_list,
    ...
    if compute_emd:
        ...
        mmd_dist = compute_mmd(sample_ref, sample_pred, kernel=gaussian_tv, sigma=1.0 )
...
class SpectreSamplingMetrics(nn.Module):
    ...
    def forward(self, generated_graphs: list, local_rank, test=False):
        ...
        if 'SA_MMD' in self.metrics_list:
            if local_rank == 0:
                print("Computing SA_MMD stats..")
            SA_MMD = SA_MMD_stats(reference_graphs, networkx_graphs[:25], is_parallel=True)

            if len(reference_graphs) > 2:
                SA_MMD_2 = SA_MMD_stats(reference_graphs[:int(len(reference_graphs)/2)], reference_graphs[int(len(reference_graphs)/2):], is_parallel=True)
            else:
                SA_MMD_2 = SA_MMD_stats(reference_graphs, reference_graphs, is_parallel=True)


            to_log['SA_MMD'] = [SA_MMD, SA_MMD_2]

            if wandb.run:
                wandb.run.summary['SA_MMD'] = [SA_MMD, SA_MMD_2]

        if 'EDA_MMD' in self.metrics_list:
            if local_rank == 0:
                print("Computing EDA_MMD stats..")
            EDA_MMD = EDA_MMD_stats(reference_graphs, networkx_graphs[:25], is_parallel=True)

            if len(reference_graphs) > 2:
                EDA_MMD_2 = EDA_MMD_stats(reference_graphs[:int(len(reference_graphs)/2)], reference_graphs[int(len(reference_graphs)/2):], is_parallel=True)
            else:
                EDA_MMD_2 = EDA_MMD_stats(reference_graphs, reference_graphs, is_parallel=True)

            to_log['EDA_MMD'] = [EDA_MMD, EDA_MMD_2]

            if wandb.run:
                wandb.run.summary['EDA_MMD'] = [EDA_MMD, EDA_MMD_2]

        if 'FE_MMD' in self.metrics_list:
            if local_rank == 0:
                print("Computing FE_MMD stats..")
            FE_MMD = FE_MMD_stats(reference_graphs, networkx_graphs[:25], is_parallel=True)

            if len(reference_graphs) > 2:
                FE_MMD_2 = FE_MMD_stats(reference_graphs[:int(len(reference_graphs)/2)], reference_graphs[int(len(reference_graphs)/2):], is_parallel=True)
            else:
                FE_MMD_2 = FE_MMD_stats(reference_graphs, reference_graphs, is_parallel=True)

            to_log['FE_MMD'] = [FE_MMD, FE_MMD_2]

            if wandb.run:
                wandb.run.summary['FE_MMD'] = [FE_MMD, FE_MMD_2]

        if 'EDA_KL' in self.metrics_list:
            if local_rank == 0:
                print("Computing EDA_KL stats..")
            EDA_KL = EDA_KL_stats(reference_graphs, networkx_graphs[:25], is_parallel=True)

            if len(reference_graphs) > 2:
                EDA_KL_2 = EDA_KL_stats(reference_graphs[:int(len(reference_graphs)/2)], reference_graphs[int(len(reference_graphs)/2):], is_parallel=True)
            else:
                EDA_KL_2 = EDA_KL_stats(reference_graphs, reference_graphs, is_parallel=True)
            

            to_log['EDA_KL'] = [EDA_KL, EDA_KL_2]

            if wandb.run:
                wandb.run.summary['EDA_KL'] = [EDA_KL, EDA_KL_2]

                

                
        if 'SA_KL' in self.metrics_list:
            if local_rank == 0:
                print("Computing SA_KL stats..")
            SA_KL = SA_KL_stats(reference_graphs, networkx_graphs[:25], is_parallel=True)

            if len(reference_graphs) > 2:
                SA_KL_2 = SA_KL_stats(reference_graphs[:int(len(reference_graphs)/2)], reference_graphs[int(len(reference_graphs)/2):], is_parallel=True)
            else:
                SA_KL_2 = SA_KL_stats(reference_graphs, reference_graphs, is_parallel=True)
            
            to_log['SA_KL'] = [SA_KL, SA_KL_2]

            if wandb.run:
                wandb.run.summary['SA_KL'] = [SA_KL, SA_KL_2]

                
        if 'FE_KL' in self.metrics_list:
            if local_rank == 0:
                print("Computing FE_KL stats..")
            FE_KL = FE_KL_stats(reference_graphs, networkx_graphs[:25], is_parallel=True)

            if len(reference_graphs) > 2:
                FE_KL_2 = FE_KL_stats(reference_graphs[:int(len(reference_graphs)/2)], reference_graphs[int(len(reference_graphs)/2):], is_parallel=True)
            else:
                FE_KL_2 = FE_KL_stats(reference_graphs, reference_graphs, is_parallel=True)
            

            to_log['FE_KL'] = [FE_KL, FE_KL_2]

            if wandb.run:
                wandb.run.summary['FE_KL'] = [FE_KL, FE_KL_2]
        ...
        if 'degree' in self.metrics_list:
            ...
            degree = degree_stats(reference_graphs, networkx_graphs[:25], is_parallel=True,
                                  compute_emd=self.compute_emd)

            if len(reference_graphs) > 2:   
                degree_2 = degree_stats(reference_graphs[:int(len(reference_graphs)/2)], reference_graphs[int(len(reference_graphs)/2):], is_parallel=True,
                                  compute_emd=self.compute_emd)
            else:
                degree_2 = degree_stats(reference_graphs, reference_graphs, is_parallel=True,
                                  compute_emd=self.compute_emd)

            to_log['degree'] = [degree, degree_2]

            if wandb.run:
                wandb.run.summary['degree'] = [degree, degree_2]

        if 'GED' in self.metrics_list:
            if local_rank == 0:
                print("Computing gaph edit distance stats..")
            GED_dict = GED_stats(reference_graphs, networkx_graphs[:25], is_parallel=True,
                                  compute_emd=self.compute_emd)
            
            to_log['GED_min'] = GED_dict["min_ged"]
            to_log['GED_max'] = GED_dict["max_ged"]
            to_log['GED_mean'] = GED_dict["mean_ged"]
            to_log['GED_std'] = GED_dict["std_ged"]
            to_log['GED_median'] = GED_dict["median_ged"]

            if wandb.run:
                wandb.run.summary['GED_min'] = GED_dict["min_ged"]
                wandb.run.summary['GED_max'] = GED_dict["max_ged"]
                wandb.run.summary['GED_mean'] = GED_dict["mean_ged"]
                wandb.run.summary['GED_std'] = GED_dict["std_ged"]
                wandb.run.summary['GED_median'] = GED_dict["median_ged"]
        ...
        if 'spectre' in self.metrics_list:
            ...
            spectre = spectral_stats(reference_graphs, networkx_graphs[:25], is_parallel=True, n_eigvals=-1,
                                     compute_emd=self.compute_emd)
        ...
        if 'clustering' in self.metrics_list:
            ...
            clustering = clustering_stats(reference_graphs, networkx_graphs[:25], bins=100, is_parallel=True,
                                          compute_emd=self.compute_emd)

            if len(reference_graphs) > 2:   
                clustering_2 = clustering_stats(reference_graphs[:int(len(reference_graphs)/2)], reference_graphs[int(len(reference_graphs)/2):], bins=100, is_parallel=True,
                                          compute_emd=self.compute_emd)
            else:
                clustering_2 = clustering_stats(reference_graphs, reference_graphs, bins=100, is_parallel=True,
                                          compute_emd=self.compute_emd)
                
            to_log['clustering'] = [clustering, clustering_2]
            if wandb.run:
                wandb.run.summary['clustering'] = [clustering, clustering_2]
...
class TFG_iker_SamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, data_loaders):
        super().__init__(data_loaders=data_loaders,
                         compute_emd=True,
                         #metrics_list=['degree', 'clustering', 'SA_MMD', 'EDA_MMD', 'FE_MMD', 'SA_KL', 'EDA_KL', 'FE_KL','GED'])
                         metrics_list=['degree', 'clustering', 'SA_MMD', 'EDA_MMD', 'FE_MMD', 'SA_KL', 'EDA_KL', 'FE_KL'])